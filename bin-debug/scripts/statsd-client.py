...
def main():
	parser = argparse.ArgumentParser(description='Single-shot commandline StatsD client')
	parser.add_argument('--host', dest='host', default='localhost', help='default localhost')
	parser.add_argument('-p', '--port', dest='port', type=int, default=8125, help='default 8125')
	parser.add_argument('-t', '--type', dest='type', choices=('count', 'incr', 'decr', 'timing', 'gauge'), default='count', help='default count')
	parser.add_argument('-k', '--key', dest='key', required=True)
	parser.add_argument('-v', '--value', dest='value', type=int, default=1, help='default 1')
	args = parser.parse_args()
	client = StatsClient(args.host, args.port)
	if args.type == 'count':
		args.type = 'incr'
	getattr(client, args.type)(args.key, args.value)
